...
class NeuralColabFilteringNet(nn.Module):
    ...
    def __init__(
        ...
        user_count,
        movie_count,
        nb_user_meatadata_features,
        n_classes=5,
        embedding_size=300,
        hidden_layers=(600, 1024, 512, 256, 128, 64, 32),
        dropout_rate=0.3,
        output_range=(1, 5),
    ):
        super().__init__()
        ...
        self.user_metadata_mlp = nn.ModuleList()
        self.user_metadata_mlp.extend(
            [nn.Linear(nb_user_meatadata_features, nb_user_meatadata_features)]  
    ...
    def _gen_MLP(self, embedding_size, hidden_layers_units, dropout_rate):
        ...
        hidden_layers.append(nn.Linear(hidden_layers_units[-1], int(self.n_classes)))
    ...
    def forward(self, user_id, user_metadata, movie_id):
        "Computes forward pass"
        user_metadata_mlp = self.user_metadata_mlp(user_metadata)
        for i, layer in enumerate(self.user_metadata_mlp):
            user_metadata_mlp = layer(user_metadata)
            user_metadata_mlp = nn.functional.relu(user_metadata_mlp)
        ...
        x = torch.cat([user_features, user_metadata_mlp, movie_features], dim=1)
