...
from modules.language_encoder import Movie_Bert
...
class Movies(Dataset):
    ...
    def __init__(self):
        ...
        # TODO: Need to you need to calculate the std/ Search what normalization does the resnet use
        self.norm = transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
        self.to_tensor = transforms.ToTensor()

        self.encoder = Movie_Bert()
    ...
    def __len__(self):
        return len(self.movies_dataset)

    def __getitem__(self, idx):

        sample = self.movies_dataset.iloc[idx]
        # user_id = sample["userId"]
        # movie_id = sample["movieId"]
        movie_descr = str(sample["title"]) + " " + str(sample["overview"])
        poster_name = os.path.join(self.poster_folder_path, sample["path"])

        image = Image.open(poster_name).convert("RGB")  # read and convert images to RGB

        image_tensor = self.norm(
            self.to_tensor(self.resize(image))
        )  # normalization and resize of poster

        # movie_image_encoder = self.image_encoder(image_tensor) # you can not do it here

        movie_rep = self.encoder(movie_descr)  # tokenization of the movie description

        return sample["movieId"], image_tensor, movie_rep
